...
boxplotsBySNPsShared <- function  (runsDir, NRUNS, CONFIGFILE, SCORESFILE, modelType, rateName, rateTitle, SNPsType) {
	...
	outFile = sprintf ("%s/out-statistics-SHARED-%s-table.csv", runsDir, SNPsType)
	write.csv (statsTable, outFile, quote=F, row.names=F)
...
main <- function () {
	...
	paramsFile = "config03.config"
	params    =  yaml.load_file(paramsFile)
	runsDir   = params$runsDir
	modelType = params$modelType
	NRUNS     = params$NRUNS

	createStatisticsPlots (NRUNS, modelType, runsDir, 0.3)
}
